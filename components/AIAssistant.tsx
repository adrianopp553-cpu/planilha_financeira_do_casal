...
import { Transaction, GroundingChunk, AIAnalysisResult } from '../types';
...
const AIAssistant: React.FC<AIAssistantProps> = ({ transactions, language }) => {
  ...
  const handleAction = async (type: 'quick' | 'deep' | 'market') => {
    ...
    try {
      ...
      setResult({ text: "Erro ao processar." });
    } finally {
      setLoading(null);
    }
  };

  // Estilo de fundo dinâmico que harmoniza com a cor primária do tema e o modo dark
  const cardStyle = {
    ...
    >
      <div className="relative z-10">
        ...
        <p className="text-white/70 mb-8 text-sm font-medium max-w-lg leading-relaxed">
          Sua inteligência financeira personalizada. Analisamos seus dados para oferecer os melhores conselhos amorosos e financeiros.
        </p>
        ...
        {result && (
          <div className="bg-black/20 p-8 rounded-[32px] border border-white/10 animate-in fade-in zoom-in-95 duration-500 backdrop-blur-md">
            <div className="prose prose-invert prose-sm max-w-none text-white/90 leading-relaxed font-medium" dangerouslySetInnerHTML={{ __html: result.text.replace(/\n/g, '<br/>') }} />
      ...
      {/* Glow dinâmico que segue a cor do tema */}
