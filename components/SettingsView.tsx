...
const SettingsView: React.FC<SettingsViewProps> = ({ settings, onUpdate, onBack, language, summary, transactions = [] }) => {
  ...
  const handleSetPin = () => {
    if (newPin.length === 4) {
      updateSetting('securityPin', newPin);
      setNewPin('');
      alert(language === 'pt' ? '🔐 PIN configurado com sucesso!' : '🔐 PIN set successfully!');
    } else {
      alert(language === 'pt' ? 'O PIN deve ter 4 dígitos.' : 'PIN must be 4 digits.');
    }
  };

  const handleRemovePin = () => {
    updateSetting('securityPin', undefined);
    alert(language === 'pt' ? '🔓 PIN removido.' : '🔓 PIN removed.');
  };

  return (
    <main className="pt-32 pb-48 px-6 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700">
      
      {/* SEÇÃO DE IMPRESSÃO (Oculta na UI) */}
      <div className="hidden print:block w-full text-black bg-white font-sans p-8">
        <h1 className="text-2xl font-black uppercase tracking-widest text-theme mb-10 border-b-4 border-theme pb-4">Extrato Consolidado</h1>
        {summary && (
          <div className="grid grid-cols-3 border-2 border-theme mb-8">
            <div className="p-4 text-center border-r-2 border-theme"><p className="text-[8px] font-black uppercase opacity-50">Entradas</p><p className="text-xl font-black">{formatCurrency(summary.totalIncome)}</p></div>
            <div className="p-4 text-center border-r-2 border-theme"><p className="text-[8px] font-black uppercase opacity-50">Saídas</p><p className="text-xl font-black text-rose-500">{formatCurrency(summary.totalExpense)}</p></div>
            <div className="p-4 text-center bg-theme/10"><p className="text-[8px] font-black uppercase opacity-50">Saldo</p><p className="text-xl font-black text-theme">{formatCurrency(summary.balance)}</p></div>
          </div>
        )}
        <table className="w-full text-[9px] border-collapse">
          <thead><tr className="bg-slate-100 border-b-2 border-theme"><th className="p-2 text-left">DATA</th><th className="p-2 text-left">DESCRIÇÃO</th><th className="p-2 text-right">VALOR</th></tr></thead>
          <tbody className="divide-y divide-slate-100">
            {transactions.map(t => (
              <tr key={t.id}><td className="p-2">{t.date}</td><td className="p-2 font-black">{t.description}</td><td className={`p-2 text-right font-black ${t.type === 'Entrada' ? 'text-emerald-600' : 'text-rose-600'}`}>{t.type === 'Entrada' ? '+' : '-'} {formatCurrency(t.amount)}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
      ...
      <div className="mb-16 no-print">
        <button onClick={onBack} className="flex items-center gap-3 text-theme font-black text-xs uppercase tracking-[0.3em] mb-6 hover:gap-5 transition-all group">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 transition-transform group-hover:-translate-x-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={4} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
          {t.saveAndBack}
        </button>
      ...
      <div className="space-y-10 no-print">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <section className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl p-10 rounded-[48px] border border-slate-200 dark:border-white/10">
            ...
            <div className="flex flex-col gap-3">
              {['pt', 'en', 'es'].map((id) => (
                <button key={id} onClick={() => updateSetting('language', id)} className={`p-5 rounded-3xl border-2 transition-all text-left group flex items-center justify-between ${settings.language === id ? 'border-theme bg-theme/5' : 'border-transparent bg-slate-50/50 dark:bg-white/5 hover:border-theme/30'}`}>
                  <span className={`font-black text-sm uppercase tracking-widest ${settings.language === id ? 'text-theme' : `${labelColor}`}`}>{id === 'pt' ? 'Português' : id === 'en' ? 'English' : 'Español'}</span>
          ...
          <section className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl p-10 rounded-[48px] border border-slate-200 dark:border-white/10">
            ...
            <div className="grid grid-cols-1 gap-4">
              {[
                ...
                <button key={theme.id} onClick={() => updateSetting('theme', theme.id)} className={`p-5 rounded-3xl border-2 flex items-center justify-between transition-all group ${settings.theme === theme.id ? 'border-theme bg-theme/5' : 'border-transparent bg-slate-50/50 dark:bg-white/5 hover:border-theme/30'}`}>
                  <div className="flex items-center gap-5">
                    <div className={`w-10 h-10 rounded-2xl ${theme.color} shadow-lg ring-4 ring-white/10`} />
                    <span className={`text-sm font-black uppercase tracking-widest ${settings.theme === theme.id ? 'text-theme' : `${labelColor}`}`}>{theme.label}</span>
                  </div>
        ...
        <section className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl p-10 rounded-[48px] border border-slate-200 dark:border-white/10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="flex items-center gap-6">
              ...
              <div>
                <p className={`font-black text-xl leading-tight ${themeOrWhite}`}>
                  {language === 'pt' ? 'Segurança do Cofre' : 'Vault Security'}
                </p>
                <p className={`text-[10px] ${secondaryLabelColor} font-black uppercase tracking-[0.3em] mt-1.5`}>
                  {settings.securityPin 
                    ? (language === 'pt' ? 'Bloqueio de acesso ativo com PIN' : 'Access lock active with PIN')
                    : (language === 'pt' ? 'Adicione um PIN para proteger seus dados' : 'Add a PIN to protect your data')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              {settings.securityPin ? (
                <button onClick={handleRemovePin} className="px-6 py-4 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-500 hover:text-white transition-all">
                  Remover PIN
                </button>
              ) : (
                <div className="flex items-center gap-2">
                  <input 
                    ...
                    className="w-32 px-4 py-4 bg-gray-100 dark:bg-white/5 border border-white/10 rounded-2xl text-center font-black tracking-[0.5em] outline-none focus:border-theme transition-all"
                  />
                  <button onClick={handleSetPin} className="px-6 py-4 bg-theme text-black rounded-2xl text-[10px] font-black uppercase tracking-widest hover:brightness-110 transition-all">
                    Ativar
                  </button>
                </div>
              )}
        ...
        <section className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl p-10 rounded-[48px] border border-slate-200 dark:border-white/10 flex items-center justify-between">
           <div className="flex items-center gap-6">
             <div className="w-16 h-16 rounded-3xl bg-theme/10 flex items-center justify-center text-3xl shadow-inner border border-theme/20">{settings.lightMode ? '☀️' : '🌙'}</div>
             <div>
               <p className={`font-black text-xl leading-tight ${themeOrWhite}`}>{language === 'pt' ? 'Alta Visibilidade' : 'High Visibility'}</p>
               <p className={`text-[10px] ${secondaryLabelColor} font-black uppercase tracking-[0.3em] mt-1.5`}>{language === 'pt' ? 'Alternar entre claro e escuro' : 'Toggle between light and dark'}</p>
             </div>
           </div>
           <button onClick={() => updateSetting('lightMode', !settings.lightMode)} className={`w-20 h-10 rounded-full transition-all relative p-1.5 ${settings.lightMode ? 'bg-theme shadow-[0_0_20px_var(--primary-shadow)]' : 'bg-slate-300 dark:bg-white/10'}`}>
        ...
        <section className="bg-slate-900 dark:bg-white/5 p-12 rounded-[56px] border border-white/10 shadow-3xl relative overflow-hidden">
          <div className="absolute top-0 right-0 p-10 opacity-5"><FCLogo className="w-48 h-48 text-white" /></div>
          <div className="relative z-10">
            <h2 className="text-xs font-black text-theme uppercase tracking-[0.5em] mb-12 flex items-center gap-4"><span className="w-2 h-2 bg-theme rounded-full"></span> {language === 'pt' ? 'Central de Arquivos' : 'Files Central'}</h2>
