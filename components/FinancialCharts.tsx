...
const FinancialCharts: React.FC<FinancialChartsProps> = ({ transactions, language }) => {
  ...
  }, [transactions]); // Update on data or re-renders
    ...
    <div className="space-y-6">
      <div className="h-64 relative">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={pieData} innerRadius={70} outerRadius={95} paddingAngle={6} dataKey="value" stroke="none" animationDuration={1000}>
            ...
            <text x="50%" y="48%" textAnchor="middle" dominantBaseline="middle" className="font-black" style={{ fontSize: '32px', fill: primaryColor }}>{expenseRatio}%</text>
            <text x="50%" y="60%" textAnchor="middle" dominantBaseline="middle" className="fill-slate-400 font-bold uppercase tracking-[0.2em]" style={{ fontSize: '10px' }}>{language === 'pt' ? 'Fluxo' : 'Flow'}</text>
          </PieChart>
        </ResponsiveContainer>
      </div>
      <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
        {pieData.map((entry, index) => (
          <div key={entry.name} className="flex items-center justify-between text-[11px] font-bold text-gray-700 bg-gray-50/50 hover:bg-white hover:shadow-sm border border-transparent hover:border-theme-light p-3 rounded-2xl transition-all duration-300 group">
