...
const FCLogo = ({ className = "h-6 w-6" }: { className?: string }) => (
  <svg viewBox="0 0 100 100" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {/* O "S" do Gráfico/Sucesso */}
    <g style={{ filter: 'url(#logoGlow)' }}>
      <path 
        d="M10 75C25 70 35 45 50 50C65 55 75 35 85 30L90 18L78 24L83 31C75 38 65 58 50 53C35 48 25 73 10 78V75Z" 
        fill="currentColor" 
        fillOpacity="0.4"
      />
      <path 
        d="M10 75C25 70 35 45 50 50C65 55 75 35 85 30" 
        stroke="currentColor" 
        strokeWidth="4" 
        strokeLinecap="round" 
      />
    ...
    {/* O Coração */}
    <path 
      d="M50 92L43.5 86C18 64.5 4 52.5 4 37C4 24.5 13.5 15 26 15C33.5 15 40.5 18.5 45 24C49.5 18.5 56.5 15 64 15C76.5 15 86 24.5 86 37C86 52.5 72 64.5 46.5 86L50 92Z" 
      stroke="currentColor" 
      strokeWidth="6" 
...
const App: React.FC = () => {
  ...
  const [settings, setSettings] = useState<AppSettings>({
    language: 'pt',
    theme: 'classic',
    lightMode: false, // Modo Escuro como padrão
    fontSize: 'medium',
    fontFamily: 'sans'
  });
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try { setTransactions(JSON.parse(savedData)); } catch (e) { console.error(e); }
    }
  ...
  useEffect(() => {
    if (isLoaded) {
      ...
      // Se lightMode for false, aplica a classe 'dark-mode' do CSS no index.html
  ...
  return (
    <div className="min-h-screen flex flex-col relative z-10" style={{ fontFamily: 'var(--app-font)' }}>
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-1 p-1 bg-gray-200/50 dark:bg-white/5 rounded-[20px] border border-black/5 dark:border-white/10 backdrop-blur-md">
            <button 
              ...
              className={`px-6 py-2.5 rounded-[16px] text-[11px] font-black uppercase tracking-widest transition-all ${view === 'home' ? 'bg-white dark:bg-theme text-theme dark:text-white shadow-xl' : 'text-gray-500 dark:text-white/60 hover:text-theme dark:hover:text-white'}`}
            >
              {t.dashboard}
            </button>
            <button 
              onClick={() => navigateTo('results')} 
              className={`px-6 py-2.5 rounded-[16px] text-[11px] font-black uppercase tracking-widest transition-all ${view === 'results' ? 'bg-white dark:bg-theme text-theme dark:text-white shadow-xl' : 'text-gray-500 dark:text-white/60 hover:text-theme dark:hover:text-white'}`}
            >
              {t.results}
            </button>
            <button 
              onClick={() => navigateTo('settings')} 
              className={`p-2.5 rounded-[16px] transition-all ${view === 'settings' ? 'bg-white dark:bg-theme text-theme dark:text-white shadow-xl' : 'text-gray-500 dark:text-white/60 hover:text-theme dark:hover:text-white'}`}
            >
        ...
        {view === 'home' ? (
          <>
            <section className="pt-48 pb-24 px-6 text-center relative">
              ...
              <button 
                onClick={() => document.getElementById('dash-content')?.scrollIntoView({ behavior: 'smooth' })} 
                className="px-14 py-7 bg-theme text-white font-black uppercase tracking-widest text-xs rounded-full shadow-2xl shadow-theme/40 hover:scale-105 active:scale-95 transition-all duration-500"
              >
                {t.startPlanning}
              </button>
            </section>
      ...
      <footer className="bg-gray-50/50 dark:bg-black/40 border-t border-black/5 dark:border-white/10 py-20 px-6 no-print backdrop-blur-xl">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-16 text-center md:text-left">
          <div className="col-span-1 md:col-span-2">
            ...
            <p className="text-gray-500 dark:text-white/60 text-base max-w-sm font-medium leading-relaxed mx-auto md:mx-0">
              O futuro financeiro do seu relacionamento elevado a uma nova dimensão. Gestão inteligente para quem planeja as estrelas.
            </p>
          </div>
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] bg-white/80 dark:bg-black/60 backdrop-blur-3xl rounded-full border border-black/5 dark:border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.4)] z-50 py-4 flex items-center justify-around no-print">
        <button onClick={() => navigateTo('home')} className={`flex flex-col items-center gap-1.5 ${view === 'home' ? 'text-theme' : 'text-gray-400'}`}>
        ...
        <button onClick={() => navigateTo('results')} className={`flex flex-col items-center gap-1.5 ${view === 'results' ? 'text-theme' : 'text-gray-400'}`}>
        ...
        <button onClick={() => navigateTo('settings')} className={`flex flex-col items-center gap-1.5 ${view === 'settings' ? 'text-theme' : 'text-gray-400'}`}>
