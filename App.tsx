...
const AuthOverlay = lazy(() => import('./components/AuthOverlay.tsx'));
...
const App: React.FC = () => {
    ...
    const loadSaved = () => {
      ...
      if (savedSettings) {
        try { 
          ...
          // Bloqueia se houver um PIN salvo
          if (parsed.securityPin) setIsLocked(true);
  ...
  return (
    <>
      ...
      <div className={`min-h-screen flex flex-col relative z-10 selection:bg-theme selection:text-white transition-all duration-1000 ${isLocked ? 'blur-2xl pointer-events-none scale-110' : 'blur-0'}`} style={{ fontFamily: 'var(--app-font)' }}>
